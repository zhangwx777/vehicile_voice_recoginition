...
"""
车辆音频数据加载器模块
包含基础数据集类、个体识别数据集类和相关工具函数
整合了原有的loader.py和individual_loader.py功能
"""
...
class VehicleSubDataset(Dataset):
    ...
    def clear_cache(self):
        ...
        if self.feature_cache:
            ...
            gc.collect()
            logger.info(f"特征缓存已清理")


class IndividualVehicleDataset(Dataset):
    """个体车辆音频数据集（从individual_loader.py整合）"""
...
def create_individual_data_loaders(data_dir, preprocessor, batch_size=None, test_size=None, val_size=None, enable_cache=False):
    """创建个体识别数据加载器（从individual_loader.py整合）"""
